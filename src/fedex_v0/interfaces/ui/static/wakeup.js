...
window._fedex_v0Sleeping   = false;
...
function resetInactivityTimer() {
    ...
    if (window._fedex_v0Sleeping) return;
...
async function enterSleepMode() {
    ...
    gsap.to(['#fedex_v0-toggle', '#globe-toggle', '#intel-toggle',
...
async function showScanResult(result) {
    ...
    if (result.recognized) {
        ...
        name.textContent = (_userFirstname || result.name || 'FEDEX_V0').toUpperCase();
...
// ═══════════════════════════════════════════════════════════════════════════
//  PHASE 5 — Déploiement UI + voix fedex-v0
// ═══════════════════════════════════════════════════════════════════════════
...
async function deployUI() {
    ...
    tl.to(['#fedex_v0-toggle', '#globe-toggle', '#intel-toggle',
            '#dashboard-nav-btn', '#settings-nav-btn', '#panels-toggle'],
    ...
    // Voix fedex-v0 (après 0.8s pour que l'UI soit visible)
    await _delay(800);
    try {
...
async function triggerWakeUp() {
    ...
    try {
        ...
        if (granted) {
            // Phase 5 — Déployer UI
            await deployUI();
            console.log('[WakeUp] Accès accordé — fedex-v0 actif');
        } else {
