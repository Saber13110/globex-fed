/* ─────────────────────────────────────────────────────────────────────
   FEDEX_V0 — Shared runtime v2 (vanilla JS, ES2017+)
   Public API attached to window.fedexV0 :

     fedexV0.mountAtmosphere()
     fedexV0.setMode(mode)                        → "home"|"workspace"|"capacites"|"config"
     fedexV0.mountRooms({ mode, pages, activePage, onNav, onMissionControl })
     fedexV0.mountTopbar({ pageTitle, crumb, onAsk, onCmdK })
     fedexV0.openCmdK() / .closeCmdK()
     fedexV0.openMissionControl() / .closeMissionControl()
     fedexV0.registerCommands(arr)
     fedexV0.notify({ kind, text })
     fedexV0.api.get(path) / .post(path, body)
     fedexV0.fmt.num(v) / .pct(v) / .relTime(d)
     fedexV0.sparkline(data, opts)
...
(function () {
  ...
  const fedex-v0 = (window.fedexV0 = window.fedexV0 || {});
  ...
  function authHeaders(extra) {
    ...
    const token = window.FEDEX_V0_API_TOKEN;
    if (token) headers.Authorization = "Bearer " + token;
    return headers;
  }

  fedexV0.api = {
    base: window.FEDEX_V0_API_BASE || "",
    async get(path) {
  ...
  fedexV0.authHeaders = authHeaders;

  /* ───────── Navigation (iframe-aware) ───────── */
  fedexV0.navigate = function (url) {
    // Si on tourne dans l'iframe, déléguer au shell parent
    if (window !== window.top && window.top.fedex-v0?.navigateFrame) {
      window.top.fedexV0.navigateFrame(url);
      return;
    }
    // Si le shell home a enregistré un handler iframe
    if (typeof fedexV0.navigateFrame === "function") {
      fedexV0.navigateFrame(url);
      return;
    }
  ...
  /* ───────── Atmosphere ───────── */
  fedexV0.mountAtmosphere = function () {
  ...
  /* ───────── Mode system ───────── */
  const MODE_META = {
    home:       { chapter: "—",  num: "00", label: "FEDEX_V0",        watermark: "" },
  ...
  /* ───────── Scene card ───────── */
  fedexV0.showSceneCard = function (chapter, title, duration) {
  ...
  fedexV0.mountRooms = function (opts) {
    _roomsOpts = opts;
    ...
    // ── Watermark ──
    fedexV0.setMode(mode);

    // ── Mission Control trigger bottom-left ──
    let mc = document.getElementById("j-rooms-mc");
    if (!mc) {
      mc = el("button", { id: "j-rooms-mc", class: "rooms-mc", onclick: () => fedexV0.openMissionControl() });
  ...
  const ROOMS = [
    ...
    { mode: "capacites", label: "Capacités",      sub: "Ce que fedex-v0 sait faire pour toi",      href: "/capabilities", chapter: "II",  pages: ["Intégrations","Skills","Routines","Ambiances","Store","Écosystème"] },
  ...
  function ensureMissionControl() {
    if (mcRoot) return;
    mcRoot = el("div", { class: "mission-overlay is-hidden", onclick: (e) => { if (e.target === mcRoot) fedexV0.closeMissionControl(); } });
    ...
    header.appendChild(el("button", { class: "mc-close", onclick: () => fedexV0.closeMissionControl(), text: "×" }));
    ...
    ROOMS.forEach(r => {
      const card = el("button", {
        class: "mission-card",
        dataset: { mode: r.mode },
        onclick: () => { fedexV0.closeMissionControl(); fedexV0.navigate(r.href); },
      });
  ...
  fedexV0.openMissionControl = function () {
    ensureMissionControl();
    mcOpen = true;
    mcRoot.classList.remove("is-hidden");
  };
  fedexV0.closeMissionControl = function () {
    if (!mcRoot) return;
    mcOpen = false;
  ...
  */
  fedexV0.mountSidebar = function (opts) {
    ...
    root.appendChild(el("div", { class: "sb-brand" }, [
      ...
      el("div", { class: "sb-brand-text" }, [
        el("span", { class: "sb-brand-name", text: "fedex-v0" }),
  ...
  /* ───────── Topbar ───────── */
  fedexV0.mountTopbar = function (opts) {
  ...
  const _navCmds = ROOMS.map(r => ({
    ...
    run: () => { fedexV0.navigate(r.href); },
  }));
  cmdkCommands = _navCmds;

  fedexV0.registerCommands = function (cmds) {
  ...
  function ensureCmdK() {
    if (cmdkRoot) return;
    cmdkRoot = el("div", { class: "cmdk-back is-hidden", onclick: (e) => { if (e.target === cmdkRoot) fedexV0.closeCmdK(); } });
    ...
    cmdkInput = el("input", {
      ...
      onkeydown: (e) => {
        if (e.key === "Escape")    { e.preventDefault(); fedexV0.closeCmdK(); }
  ...
  function currentResults() {
    ...
    if (v.startsWith(">")) {
      ...
      if (q.startsWith("ask ")) {
        const p = q.slice(4);
        return [{ kind: "ask", id: "ask", title: 'Demander : "' + p + '"', glyph: "›", group: "Faire", run: () => _askfedex-v0(p) }];
      }
  ...
  async function _askfedex-v0(prompt) {
    fedexV0.notify({ kind: "info", text: "fedex-v0 · réflexion…" });
    try {
      const r = await fedexV0.api.post("/api/agent/ask", { prompt });
      fedexV0.notify({ kind: "success", text: (r.answer || r.text || "").slice(0, 240) });
    } catch (err) {
      fedexV0.notify({ kind: "error", text: "Échec : " + err.message });
    }
  }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if ((e.metaKey || e.ctrlKey) && (e.key === "k" || e.key === "K")) {
      e.preventDefault();
      cmdkOpen ? fedexV0.closeCmdK() : fedexV0.openCmdK();
    }
    // ⌘T — Mission Control
    if ((e.metaKey || e.ctrlKey) && (e.key === "t" || e.key === "T")) {
      e.preventDefault();
      mcOpen ? fedexV0.closeMissionControl() : fedexV0.openMissionControl();
    }
    // Escape
    if (e.key === "Escape") {
      if (cmdkOpen) fedexV0.closeCmdK();
      if (mcOpen)   fedexV0.closeMissionControl();
    }
  });
  ...
  /* ───────── Notification ribbon ───────── */
  let notifStack = null;
  fedexV0.notify = function ({ kind = "info", text = "" }) {
    if (!notifStack) {
  ...
  /* ───────── Sparkline ───────── */
  fedexV0.sparkline = function (data, opts) {
    opts = opts || {};
  ...
  fedexV0.mountBottomNav = function (opts) {
