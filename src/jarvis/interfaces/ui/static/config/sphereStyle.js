...
//  retourne strictement COLOR_BASE_VEC3 (#4D148C exact).
// ═══════════════════════════════════════════════════════════════════════════
...
(function () {
    ...
    const COLOR = {
        BASE_HEX: 0x4D148C,
        BASE_HEX_STRING: '#4D148C',
        BASE_RGB_255: [77, 20, 140],
        BASE_VEC3: [0x4D / 255, 0x14 / 255, 0x8C / 255],
        HEAT_WHITE_VEC3: [1.0, 1.0, 1.0],
        HEAT_MIX_MAX: 0.70,              // mix(base, white, heat · 0.70) — heat ∈ [0,1]
        HUB_PURPLE_HEX: 0xB794F6,        // nœuds hub (lisible sur fond sombre)
        HUB_ORANGE_HEX: 0xFF6600,        // hubs majeurs FedEx
        ROUTE_HEX: 0x8B5CF6,
    };

    // ── Hubs logistiques (lat/lon → positions sur la sphère) ───────────────
    // accent: true → orange (hub majeur), sinon violet clair
    const GEO_HUBS = [
        { lat: 35.15,  lon: -90.05,  accent: true  },  // Memphis
        { lat: 49.01,  lon:   2.55,  accent: false },  // Paris CDG
        { lat: 25.25,  lon:  55.37,  accent: true  },  // Dubai
        { lat: 35.68,  lon: 139.65,  accent: false },  // Tokyo
        { lat: -23.55, lon: -46.63,  accent: false },  // São Paulo
        { lat: 33.57,  lon:  -7.59,  accent: false },  // Casablanca
        { lat: 51.51,  lon:  -0.13,  accent: false },  // London
        { lat:  1.35,  lon: 103.82,  accent: true  },  // Singapore
        { lat: 34.05,  lon:-118.24,  accent: false },  // Los Angeles
        { lat: 19.08,  lon:  72.88,  accent: false },  // Mumbai
        { lat:-33.87,  lon: 151.21,  accent: false },  // Sydney
        { lat:-26.20,  lon:  28.05,  accent: false },  // Johannesburg
    ];

    // Paires d'indices GEO_HUBS — arcs de liaison visibles
    const GEO_ROUTES = [
        [0, 1], [0, 2], [0, 5], [0, 8],
        [1, 5], [1, 6], [2, 3], [2, 7], [2, 9],
        [3, 7], [4, 8], [5, 6], [7, 9], [7, 10], [9, 11], [1, 8],
    ];

    const GEO = {
        HUB_SHELL_FACTOR: 1.03,   // légèrement au-dessus de la coque
        HUB_SIZE: 2.8,
        HUB_SIZE_ACCENT: 3.4,
        ROUTE_SEGMENTS: 24,
        ROUTE_OPACITY_IDLE: 0.22,
        ROUTE_OPACITY_THINKING: 0.48,
    };
    ...
    // ── Helpers ───────────────────────────────────────────────────────────
    // Couleur dynamique pendant CONVERGE/IGNITE. heat = 0 → base #4D148C exact.
    ...
    window.SPHERE_STYLE = Object.freeze({
        ...
        COLOR: Object.freeze({
            ...
            HUB_PURPLE_HEX: COLOR.HUB_PURPLE_HEX,
            HUB_ORANGE_HEX: COLOR.HUB_ORANGE_HEX,
            ROUTE_HEX: COLOR.ROUTE_HEX,
        }),
        GEO_HUBS: Object.freeze(GEO_HUBS.map(Object.freeze)),
        GEO_ROUTES: Object.freeze(GEO_ROUTES.map(r => Object.freeze(r))),
        GEO: Object.freeze(GEO),
