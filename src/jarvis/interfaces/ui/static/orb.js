...
(function () {
  ...
  function createOrbInternal(canvas, opts) {
    ...
    function buildRouteArc(from, to, radius, segs) {
      const v0 = new THREE.Vector3(from[0], from[1], from[2]).normalize();
      const v1 = new THREE.Vector3(to[0], to[1], to[2]).normalize();
      const out = [];
      ...
      for (let i = 0; i < segs; i++) {
        ...
        for (const t of [t0, t1]) {
          ...
          out.push(
            (a * v0.x + b * v1.x) * radius,
            (a * v0.y + b * v1.y) * radius,
            (a * v0.z + b * v1.z) * radius,
          );
        }
      }
      return out;
    ...
    // ── Hubs géo (localisation) + arcs de routes ─────────────────────
    const hubR = S.GEOMETRY.R * S.GEO.HUB_SHELL_FACTOR;
    const purpleHubPos = [];
    const orangeHubPos = [];

    (S.GEO_HUBS || []).forEach((hub) => {
      const p = latLonToVec3(hub.lat, hub.lon, hubR);
      if (hub.accent) orangeHubPos.push(p[0], p[1], p[2]);
      else purpleHubPos.push(p[0], p[1], p[2]);
    });

    function makeHubPoints(posArr, colorHex, size) {
      ...
      scene.add(pts);
      return { points: pts, geo: g, mat: m };
    }

    const hubPurple = makeHubPoints(purpleHubPos, S.COLOR.HUB_PURPLE_HEX, S.GEO.HUB_SIZE);
    const hubOrange = makeHubPoints(orangeHubPos, S.COLOR.HUB_ORANGE_HEX, S.GEO.HUB_SIZE_ACCENT);

    const routePos = [];
    (S.GEO_ROUTES || []).forEach(([ia, ib]) => {
      const ha = S.GEO_HUBS[ia];
      const hb = S.GEO_HUBS[ib];
      if (!ha || !hb) return;
      const from = latLonToVec3(ha.lat, ha.lon, hubR);
      const to   = latLonToVec3(hb.lat, hb.lon, hubR);
      routePos.push(...buildRouteArc(from, to, hubR, S.GEO.ROUTE_SEGMENTS));
    });

    const routeGeo = new THREE.BufferGeometry();
    routeGeo.setAttribute("position", new THREE.BufferAttribute(new Float32Array(routePos), 3));
    const routeMat = new THREE.LineBasicMaterial({
      color: S.COLOR.ROUTE_HEX,
      transparent: true,
      opacity: S.GEO.ROUTE_OPACITY_IDLE,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
    });
    ...
    let targetRouteOpacity = S.GEO.ROUTE_OPACITY_IDLE;
    let currentRouteOpacity = S.GEO.ROUTE_OPACITY_IDLE;

    const hubLayers = [hubPurple, hubOrange].filter(Boolean);

    function syncHubTransform(rotY, rotX, z) {
      hubLayers.forEach((layer) => {
        layer.points.rotation.y = rotY;
        layer.points.rotation.x = rotX;
        layer.points.position.z = z;
      });
      routeLines.rotation.y = rotY;
      routeLines.rotation.x = rotX;
      routeLines.position.z = z;
    }
    ...
    function animate() {
      ...
      // Hue lerp per state
      if (state === "thinking") {
        mat.color.lerp(new THREE.Color(0xFF6600), 0.015);
        lineMat.color.lerp(new THREE.Color(0xFF6600), 0.015);
        targetRouteOpacity = S.GEO.ROUTE_OPACITY_THINKING;
      } else if (state === "speaking") {
        mat.color.lerp(new THREE.Color(0xC4A0FF), 0.015);
        lineMat.color.lerp(new THREE.Color(0xC4A0FF), 0.015);
        targetRouteOpacity = S.GEO.ROUTE_OPACITY_IDLE + 0.12;
      } else {
        mat.color.lerp(new THREE.Color(S.COLOR.BASE_HEX), 0.015);
        lineMat.color.lerp(new THREE.Color(S.COLOR.BASE_HEX), 0.015);
        targetRouteOpacity = S.GEO.ROUTE_OPACITY_IDLE;
      }
      currentRouteOpacity = lerp(currentRouteOpacity, targetRouteOpacity, LK);
      routeMat.opacity = currentRouteOpacity;

      // Pulse hubs (effet « site actif »)
      if (hubOrange) {
        hubOrange.mat.opacity = 0.88 + 0.12 * Math.sin(t * 1.25);
        hubOrange.mat.size = S.GEO.HUB_SIZE_ACCENT * (0.95 + 0.1 * Math.sin(t * 0.9));
      }
      if (hubPurple) {
        hubPurple.mat.opacity = 0.85 + 0.15 * Math.sin(t * 1.1 + 0.5);
        hubPurple.mat.size = S.GEO.HUB_SIZE * (0.95 + 0.08 * Math.sin(t * 1.05 + 0.4));
      }
      ...
      if (!FROZEN) {
        ...
        electronGeo.setDrawRange(0, 0);
        lineMat.opacity = 0;
        syncHubTransform(points.rotation.y, points.rotation.x, points.position.z);
      }
      ...
      if (!FROZEN) {
        ...
        syncHubTransform(rotY, rotX, cloudZ);
      } else {
        syncHubTransform(points.rotation.y, points.rotation.x, points.position.z);
      }
