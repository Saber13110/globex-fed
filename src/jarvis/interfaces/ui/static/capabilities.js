...
(function () {
  ...
  async function renderFils() {
    const list = el("div");
    const count = await window.FilsPanel.mount(list, { api: J.api, notify: J.notify });
    const wrap = el("div");
    wrap.appendChild(ghostSec("Sessions récentes", count + " fils", null, list));
    const page = pageWrapper("fils", "Tes conversations", '<span class="v">' + count + '</span> fils', wrap);
    root.innerHTML = "";
