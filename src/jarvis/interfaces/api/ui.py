...
_UI_STATIC = Path("src/jarvis/interfaces/ui/static")
_NO_CACHE_HEADERS = {"Cache-Control": "no-store, must-revalidate", "Pragma": "no-cache"}


def _static_asset_response(rel_path: str, media_type: str) -> Response:
    path = _UI_STATIC / rel_path
    return Response(
...
async def home_js_asset() -> Response:
    """Sert home.js sans cache (prioritaire sur StaticFiles)."""
    return _static_asset_response("home.js", "application/javascript")


@router.get("/home.css", include_in_schema=False)
async def home_css_asset() -> Response:
    return _static_asset_response("home.css", "text/css")
...
def inject_client_config(html: str) -> str:
    ...
    snippet = (
        ...
        f"window.GLOBEX_OS_ENABLED={json.dumps(bool(settings.globex_os_enabled))};"
        "</script>"
    )
    marker = "</head>"
    if marker in html:
