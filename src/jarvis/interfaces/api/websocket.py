...
async def _handle_vision_event(
    ...
    if event == "gesture_open_palm":
        from jarvis.interfaces.api.globex import fetch_latest_security_incident

        report = await fetch_latest_security_incident()
        await websocket.send_json({"type": "fedex_ids_report", **report})
        logger.info(
            "Vision gesture open palm",
            found=bool(report.get("found")),
            incident_id=(report.get("incident") or {}).get("id"),
        )
        return
