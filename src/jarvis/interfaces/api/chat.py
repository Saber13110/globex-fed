...
async def voice_generate(body: VoiceGenerateRequest, request: Request) -> StreamingResponse:
    ...
    if settings.globex_os_enabled:
        ...
        session = gateway.ensure_session(body.session_id)
        session.add_message("user", voice_msg)

        async def _globex_stream() -> AsyncGenerator[str, None]:
            full = ""
            try:
                result = await invoke_globex_chat(body.message.strip(), agent_mode=True, ui_language="fr")
                full = (result.reply or "").strip()
                if result.needs_approval and result.approval_id:
                    full += f"\n\n⚠ Approbation requise (ID {result.approval_id})."
                if result.export_download and result.export_download.get("filename"):
                    full += f"\n\n📄 Export prêt : {result.export_download['filename']}"
    ...
    async def _stream() -> AsyncGenerator[str, None]:
        ...
        from jarvis.kernel.connectivity import is_lightweight_mode

        if is_lightweight_mode():
            # PERF: consolidation / autodream / BG / projets désactivés (chatbot FedEx).
            return
