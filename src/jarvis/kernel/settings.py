...
class Settings(BaseSettings):
    ...
    vision_model: str = Field(
        default="",
        description=(
            "Modèle vision optionnel (ex. llama3.2-vision:11b) — non utilisé par défaut ; "
            "llama3.2:3b reste text-only."
        ),
    )
    document_max_context_chars: int = Field(
        default=12_000,
        description="Taille max du contexte fichier envoyé à Ollama.",
    )
    document_chunk_size: int = Field(
        default=2_000,
        description="Taille des chunks pour la sélection par mots-clés.",
    )
    tesseract_cmd: str = Field(
        default="",
        description="Chemin vers tesseract.exe (auto-détecté sur Windows si vide).",
    )
    tessdata_prefix: str = Field(
        default="",
        description="Dossier tessdata ou parent (projet tessdata/ si vide).",
    )
    ocr_languages: str = Field(
        default="fra+eng",
        description="Langues OCR Tesseract, ex. fra+eng.",
    )
    ...
    # ── Globex OS (FedEx backend) ─────────────────────────────
    globex_os_enabled: bool = Field(
        default=False,
        description="Route le chat UI vers l'API Globex Agent (FedEx) au lieu du LLM local.",
    )
    globex_api_url: str = Field(
        default="http://127.0.0.1:8000",
        description="URL du backend Globex FedEx.",
    )
    globex_admin_email: str = Field(default="", description="Email admin Globex pour le proxy.")
    globex_admin_password: SecretStr = Field(
        default=SecretStr(""),
        description="Mot de passe admin Globex pour le proxy Jarvis → FedEx.",
    )
