...
# PERF OFF — vues dev extensions (réactiver si skills avec UI custom)
...
# PERF OFF — admin / budget / projets / widgets (réactiver dans include_router ci-dessous)
...
async def _warmup_ollama() -> None:
    """Précharge le modèle Ollama en RAM — évite 60-120 s au premier message."""
    import httpx
    ...
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            ...
            r.raise_for_status()
        logger.info("Ollama warmup OK", model=settings.ollama_model)
    except Exception as e:
        logger.warning("Ollama warmup échoué — le 1er message sera lent", error=str(e))
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if settings.vision_object_detection and not _turbo_local:
        asyncio.create_task(run_vision_daemon(), name="vision-daemon")

    if settings.clap_detection_enabled and not _turbo_local:
    ...
    if _turbo_local:
        logger.info("Mode turbo local — worker, scheduler, proactive et index mémoire désactivés")
        asyncio.create_task(_warmup_ollama(), name="ollama-warmup")
    else:
    ...
    if not _turbo_local:
        ...
        logger.info("Lightweight: AnalyticsRegistry ignoré")
    ...
    if not _turbo_local:
        ...
        logger.info("Lightweight: channels Telegram/Discord ignorés")
...
# PERF OFF — voix LiveKit (réactiver pour pipeline vocal)
# app.include_router(voice_router)
# PERF OFF — admin lourd (réactiver pour dashboard complet)
# app.include_router(admin_ui_router)
# app.include_router(admin_router)
# PERF OFF — projets / widgets / musique / globe / macropad / oauth
...
# ── [SURFACE] ────────────────────────────────────────────────────────────────
# PERF OFF — budget / routines / curator / connecteurs
...
# Vues dev (extensions liées) — PERF OFF
# mount_dev_views(app)
...
def main() -> None:
    ...
    """
    # reload multiprocess sous Windows : instable (KeyboardInterrupt / WinError 2).
    use_reload = settings.environment == "development" and sys.platform != "win32"
    uvicorn.run(
        "jarvis.app:app",
        host=settings.host,
        port=settings.port,
