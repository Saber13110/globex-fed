...
function Invoke-fedex-v0Python {
    param([Parameter(ValueFromRemainingArguments = $true)][string[]]$PyArgs)
    $python = Get-fedex-v0Python
    if ($python) {
        & $python @PyArgs
        return
    }
...
function Invoke-fedex-v0Run {
    $apiPort = [int](Get-DotEnvValue -Key "PORT" -Default "8000")
    $logDir = Join-Path $env:TEMP "fedex_v0"
    ...
    Get-CimInstance Win32_Process -Filter "Name = 'python.exe'" -ErrorAction SilentlyContinue |
        Where-Object { $_.CommandLine -match "fedex_v0\.(app|interfaces\.voice\.agent)" } |
    ...
    $python = Get-fedex-v0Python
    $apiCmd = if ($python) { "`"$python`" -m fedex_v0.app" } else { "uv run python -m fedex_v0.app" }
    ...
    $python = Get-fedex-v0Python
    $voiceCmd = if ($python) {
        "set PYTHONWARNINGS=ignore&& `"$python`" -m fedex_v0.interfaces.voice.agent dev --log-level info"
    } else {
        "set PYTHONWARNINGS=ignore&& uv run python -m fedex_v0.interfaces.voice.agent dev --log-level info"
    }
    ...
    if (Wait-LogMatch -LogPath $voiceLog -Pattern "fedex-v0 vocal prêt" -TimeoutSeconds 90) {
    ...
    Write-Host ""
    Write-Host "  fedex-v0 pret" -ForegroundColor Green
    ...
    try {
        ...
        Stop-Process -Name "livekit-server" -Force -ErrorAction SilentlyContinue
        Write-Host "  fedex-v0 arrete" -ForegroundColor DarkGray
    }
}
...
switch ($Command.ToLowerInvariant()) {
    ...
    "api" {
        Invoke-fedex-v0Python "-m" "fedex_v0.app"
    }
    "voice" {
        Invoke-fedex-v0Python "-m" "fedex_v0.interfaces.voice.agent" "dev"
    }
    "livekit" {
    ...
    default {
        Write-Host ""
        Write-Host "  Usage : .\fedex_v0.ps1 <commande>"
        Write-Host ""
