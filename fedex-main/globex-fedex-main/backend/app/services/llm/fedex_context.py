...
def compact_fedex_facts(shipment: dict[str, Any]) -> str:
    """Résumé texte court pour call_ollama_tracking (Phase 2) — pas de JSON volumineux."""
    tn = shipment.get("tracking_number") or "—"
    status = shipment.get("status") or "non communiqué"
    location = shipment.get("current_location") or "non communiquée"
    eta = shipment.get("estimated_delivery") or "non communiquée"
    lines = [
        f"Numéro: {tn}",
        f"Statut: {status}",
        f"Lieu: {location}",
