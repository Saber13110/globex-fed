...
def call_ollama_agent_plan(
    ...
    *,
    system_prompt: str,
    conversation_history: str | None = None,
    session_titles: str | None = None,
    ui_language: str | None = None,
) -> str:
    """Appel Ollama JSON pour le routeur Phase 4 (plan structuré)."""
    settings = get_settings()
    if not settings.llm_enabled:
        raise LlmProviderError("LLM désactivé (LLM_ENABLED=false).")

    lang = normalize_lang_code(ui_language)
    msg = (user_message or "").strip()[:1200]
    ctx_parts: list[str] = []
    if session_titles and session_titles.strip():
        ctx_parts.append(f"CONVERSATIONS RÉCENTES:\n{session_titles[:1500]}")
    if conversation_history and conversation_history.strip():
        ctx_parts.append(f"HISTORIQUE SESSION COURANTE:\n{conversation_history[:2000]}")
    ctx_block = "\n\n".join(ctx_parts)
    prompt = (
...
def call_ollama_session_summary(
    transcript: str,
    *,
    session_title: str,
    ui_language: str | None = None,
    ...
    """Résumé factuel d'une conversation (Phase 4)."""
    settings = get_settings()
    if not settings.llm_enabled:
        raise LlmProviderError("LLM désactivé (LLM_ENABLED=false).")
    ...
    if system_prompt and system_prompt.strip():
        ...
        system = (
            "Tu résumes une conversation PASSÉE client FedEx Globex.\n"
            "RÈGLES : résume UNIQUEMENT le transcript au passé (ce qui s'est dit) ; "
            "tu disposes du transcript complet ci-dessous — ne dis jamais que tu n'as pas accès aux conversations passées ; "
            "ne jamais inventer de statut colis, date ou localisation ; "
            "4 à 6 phrases factuelles maximum ; pas de markdown lourd ; "
            "INTERDIT : expliquer ce que tu peux faire, proposer de l'aide, PDF, Excel, liens ; "
            "si le transcript ne permet pas un résumé fiable, réponds exactement : RESUME_IMPOSSIBLE.\n"
            f"{language_lock_instruction(lang)}"
        )
    prompt = (
        f"===SYSTEM===\n{system}\n\n"
        f"===CONVERSATION « {session_title[:120]} »===\n"
        f"{transcript[:4000]}\n\n"
        "===RÉSUMÉ==="
    )
...
def call_ollama_tracking(
    ...
    *,
    ...
    """Appel Ollama léger pour suivi colis (Phase 2) — faits FedEx compacts, pas le gros prompt."""
    settings = get_settings()
    if not settings.llm_enabled:
        raise LlmProviderError("LLM désactivé (LLM_ENABLED=false).")
    ...
    lang = normalize_lang_code(ui_language)
    msg = (user_message or "").strip() or "Où en est mon colis ?"
    system = client_tracking_system_prompt(lang)
    facts = (fedex_facts or "").strip() or "Aucune donnée FedEx disponible."

    history_block = ""
    if conversation_history and conversation_history.strip():
        hist = conversation_history.strip()
        if len(hist) > 400:
            hist = hist[-400:]
        history_block = f"\n\n===HISTORY===\n{hist}"
