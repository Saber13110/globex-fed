"""Confirmation chat actions tickets support sensibles."""
...
from app.services.admin_client.tickets.tickets_types import PendingTicketAction
from app.services.message_attachment import unpack_message_text

TICKETS_CONFIRM_MARKER_FR = "Répondez par oui ou non pour confirmer cette action ticket"
TICKETS_CONFIRM_MARKER_EN = "Reply yes or no to confirm this ticket action"

_PENDING_RE = re.compile(
    r"\[TICKETS_ACTION_PENDING\s+(\{.*?\})\]",
    re.DOTALL,
)
...
def build_tickets_pending_marker(
    action: str,
    ticket_id: int,
    payload: dict[str, str] | None = None,
) -> str:
    data = {"action": action, "ticket_id": ticket_id, "payload": payload or {}}
    blob = json.dumps(data, ensure_ascii=False)
    return f"\n\n[TICKETS_ACTION_PENDING {blob}]"
...
def _has_tickets_marker(text: str | None) -> bool:
    blob = text or ""
    return (
        TICKETS_CONFIRM_MARKER_FR in blob
        or TICKETS_CONFIRM_MARKER_EN in blob
...
def is_tickets_action_pending_in_session(db: Session, session_id: int) -> bool:
    return latest_pending_bot_text_from_session(db, session_id) is not None


def is_tickets_action_pending(
    *,
    ...
    if _has_tickets_marker(last_bot_text):
        return True
    if _has_tickets_marker(history_text):
        return True
    if _has_tickets_marker(_last_assistant_from_history(conversation_history)):
        return True
    if db is not None and chat_session_id:
        return is_tickets_action_pending_in_session(db, chat_session_id)
    return False


def resolve_pending_ticket_action(
    *,
    history_text: str | None = None,
    conversation_history: list[Any] | None = None,
    db: Session | None = None,
    chat_session_id: int | None = None,
) -> PendingTicketAction | None:
    pending = parse_pending_ticket_action(history_text) or parse_pending_ticket_action(
        _last_assistant_from_history(conversation_history)
    )
    if pending:
        return pending
    if db is not None and chat_session_id:
        return parse_pending_ticket_action(latest_pending_bot_text_from_session(db, chat_session_id))
    return None


def parse_pending_ticket_action(text: str | None) -> PendingTicketAction | None:
    if not text:
        return None
...
def is_tickets_confirm_message(message: str) -> bool:
    return bool(_CONFIRM_RE.match((message or "").strip()))


def is_tickets_cancel_message(message: str) -> bool:
