...
# ACTIVE — Phase 2 stub
# =============================================================================
"""Chat client — Phase 3b suivi FedEx + PDF post-réponse LLM."""
...
_STRUCTURED_TRACKING_INTENTS = frozenset(
    {
        INTENT_MAP_TRACKING,
        "tabular_history",
        "scan_history",
...
def _tracking_error_reply(
    tracking_number: str,
    fedex_payload: dict[str, Any],
    ui_language: str | None,
    user: User,
) -> str:
    """Réponse déterministe si FedEx indisponible ou colis introuvable — jamais de faux statut."""
...
def _error_shipment_card(tracking_number: str, reason: str) -> dict[str, Any] | None:
    if reason != "sandbox_whitelist_denied":
        return None
...
def _trim_history_for_tracking(history: str | None, *, max_chars: int = 400) -> str | None:
    if not history or not history.strip():
        return None
    hist = history.strip()
    if len(hist) <= max_chars:
        return hist
    return hist[-max_chars:]


def _compute_phase2_reply(
    db: Session,
    user: User,
    session: ChatSession,
    message: str,
    user_msg_id: int,
    ui_language: str | None,
) -> tuple[str, str, str, str | None, str | None, dict[str, Any] | None]:
    """Phase 2 — suivi FedEx ou chat général. Retourne (reply, source, intent, tn, llm_provider, shipment)."""
    ...
    tracking_number = extract_tracking_number(message or "")
    if tracking_number and not is_plausible_tracking_number(tracking_number):
        tracking_number = None
    tracking_source = "message"
    if not tracking_number:
        tracking_number, tracking_source = resolve_tracking_for_message(
            db,
            session_id=session.id,
            user_id=user.id,
            message=message,
    ...
    if (
        tracking_number
        and not extract_tracking_number(message or "")
    ...
    if not tracking_number:
        try:
            ...
            return reply, "ollama", "general", None, "ollama", None
        except LlmProviderError:
            logger.warning("Phase 2 call_ollama_simple failed", exc_info=True)
            return (
                _ollama_fallback_reply(ui_language, user),
                "fallback",
                "ollama_error",
                None,
                None,
    ...
    if not fedex.get("available"):
        ...
        reply = _tracking_error_reply(tracking_number, fedex, ui_language, user)
        shipment_for_client = _error_shipment_card(tracking_number, reason)
        return reply, "fedex_api", reason, tracking_number, None, shipment_for_client

    shipment_data = fedex.get("shipment") or {}
    question_intent = classify_shipment_question(message)
    if question_intent in _STRUCTURED_TRACKING_INTENTS:
        reply, intent = build_shipment_reply(message, shipment_data)
        source = "fedex_api"
        llm_provider = None
    else:
        facts = compact_fedex_facts(shipment_data)
        short_history = _trim_history_for_tracking(history)
        try:
            reply = call_ollama_tracking(
                message.strip() or f"Suivi colis {tracking_number}",
                fedex_facts=facts,
                ui_language=ui_language,
            ...
            logger.warning(
                "Phase 2 call_ollama_tracking failed, fallback deterministic",
                exc_info=True,
            )
            reply, intent = build_shipment_reply(message, shipment_data)
            source = "fedex_api"
    ...
    shipment_for_client = _maybe_shipment_card(
        shipment_data,
        intent,
        tracking_source,
    )
    return reply, source, intent, tracking_number, llm_provider, shipment_for_client


def _apply_early_export_turn(early: dict[str, Any]) -> tuple[str, str, str, str | None, str | None, dict | None, dict | None]:
    return (
        early["reply"],
        early.get("source", "export"),
        early.get("intent", "export"),
        early.get("tracking_number"),
        early.get("llm_provider"),
        early.get("shipment"),
        early.get("export_download"),
    )
...
def process_user_message(
    ...
    try:
        ...
        if b64:
            ...
            if agent_turn is not None:
                (
                    ...
                    shipment_for_client,
                    export_download,
                ) = _apply_early_export_turn(agent_turn)
            else:
                early_turn = (
                    handle_excel_only_followup_turn(db, user, session, message, ui_language)
                    or handle_conversation_pdf_turn(
                        db, user, session, message, user_msg.id, ui_language
                    )
                    or handle_pdf_only_followup_turn(db, user, session, message, ui_language)
                )
                # Phase 3a désactivé — handle_client_export_turn (mots-clés pré-chat) remplacé par pdf_postprocess
                if early_turn is not None:
                    (
                        ...
                        shipment_for_client,
                        export_download,
                    ) = _apply_early_export_turn(early_turn)
                else:
                    (
                        reply,
                        source,
        ...
        result["agent_mode"] = False
        if export_download:
            result["export_download"] = export_download
        return result
    except Exception:
        logger.exception("chatbot Phase 3 failure")
        db.rollback()
        return {
