...
def store_client_excel_blob(
    *,
    ...
    """Stocke un Excel client pour téléchargement via export_token."""
    _purge_expired()
    token = uuid.uuid4().hex
    _cache[token] = (
        time.time() + _TTL_SECONDS,
    ...
    logger.info(
        "[EXPORT] client_excel_blob_stored token=%s module=%s bytes=%s",
        token[:8],
        module,
        len(xlsx_bytes),
    )
